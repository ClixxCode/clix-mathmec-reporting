...
export function ExecutiveSummaryDesktop() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        // Auto-expand when scrolling back to original position
        if (!scrolledPast) {
  ...
  const panelContent = (
    <div className="bg-gradient-to-br from-blue-50 to-blue-100/50 rounded-2xl border border-blue-100 p-6 relative shadow-lg">
      {/* Only show collapse button when sticky */}
      {isSticky && !isCollapsed && (
        <button
          onClick={() => setIsCollapsed(true)}
          className="absolute top-4 right-4 p-1.5 rounded-lg hover:bg-blue-200/50 transition-colors text-gray-500 hover:text-gray-700"
          aria-label="Collapse Key Findings"
        >
      ...
      <div className="flex items-start gap-4">
        ...
        <div className={`space-y-4 ${isSticky ? 'pr-6' : ''}`}>
  ...
  return (
    <>
      ...
      {/* Fixed position panel - visible when sticky and not collapsed */}
      <div
        className="fixed top-20 left-1/2 -translate-x-1/2 z-40 w-full max-w-[1280px] px-4 transition-all duration-300"
        style={{
          opacity: isSticky && !isCollapsed ? 1 : 0,
          transform: `translateX(-50%) ${isSticky && !isCollapsed ? 'translateY(0)' : 'translateY(-20px)'}`,
          pointerEvents: isSticky && !isCollapsed ? "auto" : "none",
        }}
      >
        {panelContent}
      </div>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-50 p-4 rounded-full bg-blue-600 shadow-lg shadow-blue-600/30 hover:bg-blue-700 transition-all duration-300 flex items-center gap-2"
        style={{
          opacity: isCollapsed && isSticky ? 1 : 0,
          transform: isCollapsed && isSticky ? "scale(1)" : "scale(0.8)",
          pointerEvents: isCollapsed && isSticky ? "auto" : "none",
        }}
        aria-label="Show Key Findings"
      >
        <Lightbulb className="w-5 h-5 text-white" />
        <span className="text-white text-sm font-medium pr-1">Key Findings</span>
      </button>
    </>
  );
}
